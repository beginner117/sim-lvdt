...
class Analysis():
    def __init__(self, filename: str = None, n_steps=10, stepsize=0.1, inncoil_offset=-0.5, inn_height=18, inncoil_rad=21,
                 out_height=13.5, outcoil_rad=31.5, outcoil_distance=14.5, outercoil_layers=5, fit=None):
        self.filename = filename
        self.Nsteps = n_steps
        self.stepsize = stepsize
        self.InnCoil_off = inncoil_offset
        self.inn_height = inn_height
        self.inn_rad = inncoil_rad
        self.out_height = out_height
        self.out_rad = outcoil_rad
        self.out_dist = outcoil_distance
        self.outer_layers = outercoil_layers
        self.fit = fit
    def simulate(self):
        ...
        pre_simulation = design.Simulation(Nsteps=self.Nsteps, stepsize=self.stepsize, inncoil_offset=self.InnCoil_off,
                                           data_file =self.filename, fit_points=self.fit)
        sensor = design.Sensortype(InnCoilCurrent = 0.02, Simfreq = 10000, OutCoilCurrent = 0)
        femm.mi_probdef(sensor.para()[1], 'millimeters', 'axi', 1.0e-10)
        wire = design.Wiretype("32 AWG", "32 AWG")
        geo = design.Geometry(inn_ht=self.inn_height, inn_rad=self.inn_rad, inn_layers=6, inn_dist=0,
                              out_ht=self.out_height, out_rad=self.out_rad, out_layers=self.outer_layers,
                              out_dist=self.out_dist, mag_len=40, mag_dia=10, ver_shi=0)
        ...
        class Results():
            def __init__(self):
                pass
            print("inn vol : ", np.array(modelled.InnCoil_Voltages))
            ...
            Norm_OutCoil_Signals = (abs(modelled.UppOutCoil_Voltages) - abs(modelled.LowOutCoil_Voltages)) / abs(np.array(modelled.InnCoil_Voltages))
            OutCoil_Signals = (abs(modelled.UppOutCoil_Voltages) - abs(modelled.LowOutCoil_Voltages))
            print("signal1 : ", OutCoil_Signals)
            print("Normalised out coil signals :", Norm_OutCoil_Signals)

            if self.Nsteps >= 1:  # Curve fit is only possible with 2 or more datapoints
                def linfunc(x, a, b):
            ...
            if self.fit:
                a = mid - pre_simulation.parameters()[4]
                b = mid + pre_simulation.parameters()[4] + 1
        ...
        class Linearity():
            ...
            def __init__(self):
                pass
            if req_plots.Norm_fiterror == 1 and self.fit:
        ...
        linearity = Linearity()
        if self.filename:  # Only save the data when a filename is specified
            np.savez(self.filename, IC_positions=modelled.InnCoil_Positions, IC_voltages=modelled.InnCoil_Voltages,
                     UOC_voltages=modelled.UppOutCoil_Voltages, LOC_voltages=modelled.LowOutCoil_Voltages,
                     UOC_flux=modelled.UppOutCoil_Flux, LOC_flux=modelled.LowOutCoil_Flux)
            print('file saved')
