...
class Analysis:
    ...
    def simulate(self):
        ...
        input_par1 = {'TotalSteps_StepSize_Offset': self.sim_range,
                      'uppercoil Diameter_Insulation_Wiretype': wire.prop_out(),
                      'innercoil Diameter_Insulation_Wiretype': wire.prop_inn(), 'Innercoil_current': sensor.para()[0],
                      'Frequency': sensor.para()[1],
        ...
        if self.default == 'yes':
            ...
            input_par2 = {'inn coil_height': 5.2, 'inn coil_radius': 10, 'inn coil_layers': 1, 'inn coil_distance': 0,
                          'out coil_height': 5.2, 'out coil_radius': 10, 'out coil_layers': 1,
                          'out coil_distance': 0, 'mag_length': 40, 'mag_diameter': 10, 'ver_shi': 0}
            geo = design.Geometry(input_par2['inn coil_height'], input_par2['inn coil_radius'],
                                  input_par2['inn coil_layers'], input_par2['inn coil_distance'],
                                  input_par2['out coil_height'], input_par2['out coil_radius'],
                                  input_par2['out coil_layers'], input_par2['out coil_distance'],
        ...
        for i in range(0, pre_simulation.parameters()[0] + 1):
            ...
            for item in range(0, geo.outcoil()[2]):
                for j in range(0, turns_per_layer):
                    ...
                    mag_field_upper.append([grid_pt[0], grid_pt[1], b_field])
                    b_field_lower = femm.mo_getb(grid_pt_lower[0], grid_pt_lower[1]);
                    mag_field_lower.append([grid_pt_lower[0], grid_pt_lower[1], b_field_lower])
            ...
            mag_field_uppercoil.append(mag_field_upper);
            mag_field_lowercoil.append(mag_field_lower)
        ...
        if self.save:
            np.savez_compressed(self.filename, Design=input_par2, Input_parameters=input_par1,
                                ...
                                Innercoil_config=position.inncoil(),
                                IC_forces=inn_prop['Inncoil_force'],
                                Mag_forces=mag_prop['Magnet_forces'], semi_analytical_def=for_def,
                                semi_analytical_imp=imp_force,
                                Magnetic_field_upper=mag_field_uppercoil, Magnetic_field_lower=mag_field_lowercoil,
                                IC_currents=inn_prop['Inncoil_current'],
                                IC_voltages=inn_prop['Inncoil_voltage'],
                                IC_positions=inn_prop['Inncoil_position'], IC_flux=inn_prop['Inncoil_flux'])