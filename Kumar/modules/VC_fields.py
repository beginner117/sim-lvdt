...
class Analysis:
    ...
    def simulate(self):
        ...
        # femm.mi_moverotate(position.magnet()[2]/2, (position.magnet()[0]+position.magnet()[1])/2, -15)
        for_def = []
        for_imp = []
        ...
        for i in range(0, pre_simulation.parameters()[0] + 1):
            ...
            for item in range(0, geo.outcoil()[2]):
                for j in range(0, turns_per_layer):
                    grid_pt = [35 + (item * 0.238), 34 - (j * 0.238)]
                    grid_pt_lower = [35 + (item * 0.238), -34 + (j * 0.238)]
                    ...
                    g = (6.28 * grid_pt[0] * b_field[0]) / (10 ** 3)
                    g_lower = (6.28 * grid_pt_lower[0] * b_field_lower[0]) / (10 ** 3)
                    def_force.append(g - g_lower)

                    angle = self.parameter1
                    ...
                    f = (6.28 * grid_pt[0] * c[0]) / (10 ** 3)
                    f_lower = (6.28 * grid_pt_lower[0] * c_lower[0]) / (10 ** 3)
                    imp_force.append(f-f_lower)

            print('default force:', sum(def_force))
            print('updated force:', sum(imp_force))
        ...
        print('angle, default force1:',self.parameter1, for_def)
        print('angle, updated force1:',self.parameter1,  for_imp)

        # plt.quiver(gri_x, gri_y, mag_fie_x, mag_fie_y, color = 'g', label = 'default', alpha = 0.5)
        # plt.quiver(gri_x, gri_y, rot_x, rot_y,color = 'b', label = 'rotated', alpha = 0.3)
        # plt.quiver(gri_x_lower, gri_y_lower, mag_fie_x_lower, mag_fie_y_lower, color = 'g', alpha = 0.5)
        # plt.quiver(gri_x_lower, gri_y_lower, rot_x_lower, rot_y_lower, color = 'b', alpha = 0.3)
        # plt.title('Magnetic Field (with angle -{})'.format(angle))
        plt.quiver(gri_x, gri_y, mag_fie_x, np.zeros(56*5), color='g', label='default', alpha=0.5)
        plt.quiver(gri_x, gri_y, rot_x, np.zeros(56*5),color = 'b', label = 'rotated', alpha = 0.3)
        plt.quiver(gri_x_lower, gri_y_lower, mag_fie_x_lower,  np.zeros(56*5), color = 'g', alpha = 0.5)
        plt.quiver(gri_x_lower, gri_y_lower, rot_x_lower, np.zeros(56*5), color = 'b', alpha = 0.3)
